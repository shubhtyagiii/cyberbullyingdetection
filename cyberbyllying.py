...
Original file is located at
    https://colab.research.google.com/drive/16lDGihTgCFOj7DnaSyBG9GR46o-6EtQU
"""
...
mean_values = df_scraped.mean()
df_scraped.fillna(value= mean_values, inplace= True)

df_public.drop_duplicates(inplace = True)
m = df_public.mean()
df_public.fillna(value= m, inplace= True)
df_scraped.head(2)
df_public.head(2)
...
def pipeline(learner_list, X_train, y_train, X_test, y_test):
    ...
    for learner in learner_list:
        result = {}  # Store results for each learner

        # Store the learner name
        result['Algorithm'] = learner.__class__.__name__

        # Fit the learner and measure training time
        start_time = time()
        ...
        # Store training time
        result['Training Time'] = end_time - start_time
        ...
        result['Prediction Time'] = end_time - start_time
...
textstr = '\n'.join(
    ['Best Accuracy: {:.3f} - {}'.format(best_acc['Accuracy: Test'].values[0], best_acc['Algorithm'].values[0]),
     ...
     'Best Precision: {:.3f} - {}'.format(best_precision['Precision: Test'].values[0],
                                          best_precision['Algorithm'].values[0]),
...
textstr = '\n'.join(('Best Training Time: {:.3f} - {}'.format(best_train_time['Training Time'].values[0],
                                                              best_train_time['Algorithm'].values[0]),
                     'Worst Training Time: {:.3f} - {}'.format(worst_train_time['Training Time'].values[0],
                                                               worst_train_time['Algorithm'].values[0]),
                     'Best Prediction Time: {:.3f} - {}'.format(best_prediction_time['Training Time'].values[0],
                                                                best_prediction_time['Algorithm'].values[0]),
                     'Worst Prediction Time: {:.3f} - {}'.format(worst_prediction_time['Training Time'].values[0],
                                                                 worst_prediction_time['Algorithm'].values[0])))
